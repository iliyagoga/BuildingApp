...
const Manager=observer((mode)=>{
    ...
    <AppBar sx={{position:"static"}} component={Paper}>
        <Toolbar sx={{display: "flex", gap:"10px"}}>
            <Button variant={mode['mode']=="appls"?"outlined":"contained"} onClick={()=>{nav(routes.manager.mean+routes.manager.objects)}}>
                Объекты
            </Button>
            <Button variant={mode['mode']=="objects"?"outlined":"contained"} onClick={()=>{nav(routes.manager.mean+routes.manager.appls)}}>
                Заявки
            </Button>
        </Toolbar>
    </AppBar>
